    ...
    public void appendTooltip(ItemStack itemStack, World world, List<Text> tooltip, TooltipContext tooltipContext) {
        ...
        if (nbtCompound != null) {
            ...
            if (design.length() <= TOOLTIP_DESIGN_LENGTH) {
                tooltip.add(Text.translatable("item.numismacraft.coin.tooltip_1_1", design));
            } else {
                while (design.length() > TOOLTIP_DESIGN_LENGTH) {
                    ...
                    tooltip.add(Text.translatable("item.numismacraft.coin.tooltip_1_" + count, sub));
                    design = rem;
                    count++;
                }
                if(design.length() > 0){
                    tooltip.add(Text.translatable("item.numismacraft.coin.tooltip_1_" + count, design));
                }
            }
            tooltip.add(Text.translatable("item.numismacraft.coin.tooltip_2", year, mint));
        }
